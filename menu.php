...
<div class=" col-12  col-9-lrg ">
<?php 
$title1=mysqli_query($cnx,'select * from menu');
 while($title=mysqli_fetch_array($title1)){
  echo '<section  class="scrollto-block">


  <h2 class="title-lined">'.$title["menu"].'</h2>
  <div class="grid">';
 $produits1=mysqli_query($cnx,'select * from produits where categorie="'.$title["menu"].'"');
    while($produits=mysqli_fetch_array($produits1)){
        echo '<div class="col-6 col-4-med col-4-lrg">
		<div class="callout callout--product">
				<picture class="" data-label=>
					<img src="'.$produits["image"].'" alt="'.$produits['produit'].'"/>
				</picture>
				<p class=>'.$produits['produit'].'</p>
				<a href="paiment.php?produit='.$produits["id"].'"> <button type="button" class="btn btn-success">commander</button></a>
			</a>
		</div>
	</div>';
	}
